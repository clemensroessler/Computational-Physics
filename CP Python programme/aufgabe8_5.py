"""Quantenmechanik von 1D-Potentialen III: Phasenraumdarstellung
...
Anfagspunkt und stellt diese dann grafisch in der Husimi-Darstllung innerhalb 
eines Phasendiagramms zusammen mit einem Konturplot dar. 
"""
...
def gausssches_wellenpaket(x, x_0=0, delta_x=0.1, h_eff=0.06, p_0=0):
    ...
    """
    return (1 / (delta_x * np.sqrt(np.pi)))**0.5 * \
        np.exp(-(x - x_0)**2 / (2 * delta_x**2) + 1j * p_0 * x / h_eff)
...
def zeitentwickelte_husimi_darstellung(phi_0, t_array, dx, eigen_energien,
                                       eigen_funktionen, k_zustand_tensor,
                                       h_eff=0.06):
    """
...
def neues_Wellenpacket(event, ax, eig_array, x_array, p_array, t_array, dx,
                       eigen_energien, eigen_funktionen, k_zustand_tensor,
                       h_eff=0.06):
    """
    Berechnet die Zeitentwicklung eines neuen Wellenpakets basierend auf 
    einem Mausklickereignis und stellt es dar.
    ...
    if event.button == 1 and event.inaxes == ax and mode == "":
        ...
        hus_matrix_array = zeitentwickelte_husimi_darstellung(
            phi_0, t_array, dx, eigen_energien, eigen_funktionen,
            k_zustand_tensor, h_eff)

        # Darstellung der Husimi-Darstellung
        hus_img_plot = ax.imshow(hus_matrix_array[0], aspect="auto", extent=(
            x_array[0], x_array[-1], p_array[0], p_array[-1]))  # Anfangsplot
...
def main():
    ...
    # gibt die Anzahl an Punkten für die Eigenfunktionen an
    N_eig = 100
    ...
    print(
        f"""
    Verwendete Parameter: Das betrachtete Intervall wurde von x = {x_min} bis 
    x = {x_max} gewählt. Die Genauigkeit der Lösung der Wellenfunktion wurde 
    durch die Anzahl an Punkten für die Eigenfunktionen mit {N_eig} wählt. 
    Das effektives Plancksche Wirkungsquantum h_eff wurde mit {h_eff} 
    vorgegeben und der Potentialparameter A wurde ebenfalls gegeben mit A = 
    {A}. Die Anzahl der Zeitpunkte pro Zeiteintervall wurde auf 
    {anz_zeit_per_intervall} gesetz und das Zeitintervall beginnt bei 
    {t_start} und endet bei {t_end}. Als Niveaus für den Konturplot wurden 
    {contour_werte} als Energieniveaus gewählt.
    """
    )
    ...
    print(
        """
    Sie sehen ein Phasendiagramm welches Energieniveaus eines
    Doppelmuldenpotentials anzeigt. Sie können nun einen Startpunkt per Klick 
    auswählen um die Zeitentwicklung eines ein symmetrischen kohärenten 
    Zustandes an diesem Punkt zu beginnen. Daraufhin wird Ihnen die 
    Husimi-Darstellung dieses von Ihnen erzeugten kohärenten Zustandes 
    angezeigt.
    """
    )
    ...
    ax.set_title(
        "Phasendiagramm der Zeitentwicklung im Doppelmuldenpotential")
    ax.set_xlabel("Ort x")
    ax.set_ylabel("Impuls p")

    # Zeichne Konturlinien und labele sie
    ...
    klick_funktion = functools.partial(
        neues_Wellenpacket, ax=ax, eig_array=eig_array, x_array=x_array,
        p_array=p_array, t_array=t_array, dx=dx,
        eigen_energien=eigen_energien, eigen_funktionen=eigen_funktionen,
        k_zustand_tensor=k_zustand_tensor, h_eff=h_eff)
...
if __name__ == "__main__":
    ...
    """
    a) 
    Der Hauptunterschied der quantenmeschanichen und klassischen Dynamik 
    liegt daran, dass wir hier nicht nur die Dynamik eines einzelnen Punkt 
    betrachten sondern quasi die Dynamik vieler Punkte mit unterschiedlichen 
    Wichtungen gleichzeitig. Beim Start in einem Minimum bleibt der Hauptteil 
    des Wellenpacketes auch im Minimum. Beim Start auf einer der äußeren 
    Energieniveaus (wie z.B. E = 2), bleibt das Wellenpacket auf diesem 
    Energieniveau ist aber nicht auf einem Phasenpunkt lokalisiert sondern 
    verteilt sich über das gesamte Energieniveau. 
    
    b) Für h_eff = 0.01 beobachtet man eine deutliche Verringerung der Breite 
    des Wellenpacketes. Außerdem ist eine in p periodische Wiederholung der 
    Bewegung des Wellenpacketes im Phasenraum zu sehen.
