...
def kohaerenter_zustand_tensor(eig_array, x_array, p_array, h_eff=0.06):
    ...
    path = np.einsum_path("i,j", x_array,
                          eig_array, optimize='optimal')[0]
    x_matrix = np.einsum("i,j", x_array, np.ones(
        eig_array.size), optimize=path)
    eig_matrix = np.einsum("i,j", np.ones(x_array.size),
                           eig_array, optimize=path)
    x_eig_matrix = x_matrix - eig_matrix
    x_eig_exp_matrix = np.exp(x_eig_matrix ** 2 / (-2 * h_eff))
    # p steht eigentlich für den index k, jedoch muss für die einsum funktion mit i begonnen werden
    p_eig_matrix = np.einsum("i,j", p_array, eig_array)
    p_eig_exp_matrix = np.exp(1j / h_eff * p_eig_matrix)
    k_zustand_tensor = np.einsum(
        ",ij,kj->jik", faktor, x_eig_exp_matrix, p_eig_exp_matrix)
...
def zeitentwickelte_husimi_darstellung(phi_0, eig_array, x_array, p_array, t_array, potential, dx, h_eff=0.06):

    # Berechnet Eigenenergien und Eigenfunktionen
    eigen_energien, eigen_funktionen = qm.diagonalisierung(
        h_eff, eig_array, potential)
    ...
    integrand = np.einsum("jik,lj->ljki", np.conj(k_zustand_tensor), phi_t_x)
...
def neues_Wellenpacket(event, ax, x_array, p_array, t_array, potential, h_eff=0.06, N_eig=200):
    """
    ...
    if event.button == 1 and event.inaxes == ax and mode == '':
        ...
        eig_array, dx = qm.diskretisierung(
            x_array[0], x_array[-1], N_eig, retstep=True)
        ...
        hus_matrix_array = zeitentwickelte_husimi_darstellung(
            phi_0, eig_array, x_array, p_array, t_array, potential, dx=dx, h_eff=h_eff)
...
def main():
    ...
    klick_funktion = functools.partial(
        neues_Wellenpacket, ax=ax, x_array=x_array, p_array=p_array, t_array=t_array, potential=potential_von_A, h_eff=h_eff, N_eig=N_eig)
