...
def Polarradius(t):
    """gibt den Radius r der Spirale zurück in Abhängigkeit vom Parameter t"""
    return (1/2)**t

def Polarwinkel(t):
    """gibt den Polarwinkel phi der Spirale zurück in Abhängigkeit vom Parameter t"""
    return 2*t*math.pi
...
def SpiralenErzeugende(Mitte=np.zeros(2),Windungen=5,Datenpunkte=1000):
    ...
    t=np.linspace(0.0,Windungen,Datenpunkte) #numpy array des parameter t für gegebene Anzahhl an Windungen und Datenpunkten 
    Radius=Polarradius(t) # numpy array der Radien r für alle t
    Winkel=Polarwinkel(t) # numpy array der Winkel phi für alle t
    xArray,yArray=PolarZuKartesisch(Radius,Winkel) # Umwandlung des Radius und des Winkels in x und y numpy arrays
...
def wenn_maus_geklickt(event,ax, Windungen=5,Datenpunkte=1000):
    ...
    if event.button == 1 and event.inaxes and mode == '':
        ...
        x, y = SpiralenErzeugende(Mitte=np.array([xpos,ypos]),Windungen=Windungen,Datenpunkte=Datenpunkte)
...
def main():
    ...
    x,y=SpiralenErzeugende()
    ax.plot(x, y, ls='-', lw=1, c='b')  
    
    #definiert Subfunktion von "wenn_maus_geklickt"-Funktion, bei welcher ax festgelegt ist
    klick_funktion = functools.partial(wenn_maus_geklickt, ax=ax)
