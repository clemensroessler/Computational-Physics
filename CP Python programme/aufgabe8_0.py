...
def kohaerenter_zustand_tensor(x_array, p_array, h_eff=0.06):
    ...
    p_x_matrix = np.einsum("k,j", p_array, x_array)
...
def husimi_darstellung(phi, x_array, p_array, dx, h_eff=0.06):
    husimi_matrix = np.zeros((x_array.size, p_array.size))
    k_zustand_tensor = kohaerenter_zustand_tensor(
        x_array, p_array, h_eff)
    for i in range(x_array.size):
        for k in range(p_array.size):
            # k_zustand = kohaerenter_zustand(
            #   x_array, x_array[i], p_array[k], h_eff)
            k_zustand = k_zustand_tensor[:, i, k]
            integrand = np.conj(k_zustand) * phi
            stammfunktion = np.sum(dx * integrand) - \
                dx / 2 * (integrand[0] + integrand[-1])
            husimi_punkt = np.abs(stammfunktion) ** 2 / h_eff
            husimi_matrix[i, k] = husimi_punkt
    return husimi_matrix
...
def zeitentwickelte_husimi_darstellung(phi_0, x_array, p_array, t_array, dx, potential, h_eff=0.06):
    ...
    integrand = np.einsum("jik,lk->ljik", np.conj(k_zustand_tensor) * phi_t_x)

    stammfunktion = np.sum(dx * integrand, axix=1) - \
        dx / 2 * (integrand[:, 0, :, :] + integrand[:, -1, :, :])
...
def main():
    ...
    phi = kohaerenter_zustand(x_array)
    ...
    # kohaerenter_zustand_tensor(x_array, p_array, h_eff)

    hus_img = husimi_darstellung(
        phi, x_array, p_array, dx)
    ax.imshow(hus_img, aspect="auto",
              extent=(*x_intervall, *p_intervall))
