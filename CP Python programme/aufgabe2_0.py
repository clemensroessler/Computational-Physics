...
def gesamt_iteration_standardabbildung(offset=np.zeros(2), kickstärke=2.6, anz_iterationen=1000):
    ...
    # übergibt die Startwerte in die Arrays
    phi_array[0] = offset[0]
    p_array[0] = offset[1]
...
def main():
    ...
    # Achsengrenzen
    ax.axis([0, 2.0 * np.pi, -np.pi, np.pi])
...
"""Als Aufgabe wurde gestellt den Phasenraum fur verschiedene Werte des Parameters K: K = 0; 0,2; 0,9; 2,1; 2,5; 6,0; 6,5 zu betrachten. Fuer K=K = 0; 0,2; 0,9; 2,1; 6,0; 6,5 waren nur chaotische Bereiche zu finden. 
Fuer K=2.5 waren reguläre Bereich in Form von Inseln zu sehen.
Diese waren umgeben von Bereichen chaotischer Dynamik. Ich habe insgesamt vier Gruppen an solchen regulären Inseln entedeckt, wobei ich jede Fixpunktgruppe als jeweils eine Gruppe gezählt habe.
...
Im Bereich p/p_0 von 0.290 bis 0.302 und phi von 2.135 bis 2.141 kann man den Grenzübergang von regulärer zu chaotischer Dynamik beobachten. Hier war besonders interessant zu sehen, dass die im zwischen diesem Bereich und der anderen vorher erwähnten Gruppe zwar chaotisch sind, dennoch eine gewisse Ordnung nahe dem regulären Bereich haben.
